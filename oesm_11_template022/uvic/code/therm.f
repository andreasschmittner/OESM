! source file: /data2/aschmitt/models/OESM/0.1/oesm_11_template022/uvic/updates/therm.F
...
! ericmod adding dawei's ice flux into ocean

!-----------------------------------------------------------------------
!           LDW: add IS frozen runoff as sea ice
!-----------------------------------------------------------------------
! ericnote: dh is added to hice, so in units m???
!               and runfrz is in units cm/s
!              dh = dh + runfrz(i,j) * dts
! end ericmod
...
!           SBT: Adding frozen runoff after sea-ice salt flux
!           calculated
            dh = dh + runfrz(i,j) * dts
...
!-----------------------------------------------------------------------
     ...
     &                        - rhosno*dhflxs
              stf_si(i,j,1) = ff*dhflxi
     &                          +ffs*dhflxs
              stf_si(i,j,2) = - rhoice*dhflxi
     &                         - rhosno*dhflxs
            endif
