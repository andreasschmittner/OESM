! source file: /data2/aschmitt/models/OESM/0.1/oesm_11_t020_short/uvic/updates/gosbc.F
...
!     this could go in common block so it doesn't have to be done twice
!     (here and in rivmodel.F)
      real icemsk(imt,jmt)
!     interactive land ice mask = 1 (no ice) = 0 (ice)
...
!-----------------------------------------------------------------------
      ...
      call rivmodel
      globaldisch = 0.
      globaldisch_unmskd = 0.
      globaldisch_runliq = 0.
      do j=jsp1,jem1
        do i=isp1,iem1
          if (kmt(i,j) .ne. 0.) then
            globaldisch = globaldisch + disch(i,j)*dxt(i)*dyt(j)*cst(j)
            globaldisch_unmskd = globaldisch_unmskd
     &                         + disch_unmskd(i,j)*dxt(i)*dyt(j)*cst(j)
            globaldisch_runliq = globaldisch_runliq
     &                         + disch_runliq(i,j)*dxt(i)*dyt(j)*cst(j)
          endif
        enddo
      enddo
