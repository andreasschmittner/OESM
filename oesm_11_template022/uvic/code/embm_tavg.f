! source file: /data2/aschmitt/models/OESM/0.1/oesm_11_t020_short/uvic/updates/embm_tavg.F
...
!-----------------------------------------------------------------------
     ...
     &, 'river discharge', 'river_discharge_flux', 'kg m-2 s-1')
      call defvar ('F_rivdis_unmskd', iou, 3, it, -c1, c1, ' ', 'F'
     &, 'river discharge unmasked', 'river_disch_unmskd', 'kg m-2 s-1')
      call defvar ('F_rivdis_runliq', iou, 3, it, -c1, c1, ' ', 'F'
     &, 'river discharge runliq', 'river_disch_runliq', 'kg m-2 s-1')
...
!-----------------------------------------------------------------------
     ...
     &,                         ws, runoff, disch_unmskd, disch_runliq
     &,                         soilm, surf
     &,                         tice, hice, aice, hsno
     &,                         uice, vice, xint, yint
...
!=======================================================================
      ...
      real disch_unmskd(ids:ide,jds:jde)
      real disch_runliq(ids:ide,jds:jde)
...
!-----------------------------------------------------------------------
     ...
     &,     c1, c0)
      ...
      tmpij(ils:ile,jls:jle) = disch_unmskd(ils:ile,jls:jle)
      call putvara ('F_rivdis_unmskd', iou, ln, ib, ic, tmpij, p1, c0)
      tmpij(ils:ile,jls:jle) = disch_runliq(ils:ile,jls:jle)
      call putvara ('F_rivdis_runliq', iou, ln, ib, ic, tmpij, p1, c0)
