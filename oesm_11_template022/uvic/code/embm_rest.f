! source file: /data/home/mortense/models/coupled_uvic-pim/uvic-pim_11_atmC_cntrl/uvic/updates/embm_rest.F
...
c        only update Southern Hemisphere for now since only Antarctic
c        ice sheet is interactive
c        tmsk(ils:ile,jls:jle) = tmpij(ils:ile,jls:jle)
        tmsk(ils:ile,jls:int(jle/2)) = tmpij(ils:ile,jls:int(jle/2))
        tmpij(ils:ile,jls:jle) = hicel(ils:ile,jls:jle,2)
        call getvara ('hicel', iou, ln, ib, ic, tmpij, c1, c0)
c        hicel(ils:ile,jls:jle,2) = tmpij(ils:ile,jls:jle)
        hicel(ils:ile,jls:int(jle/2),2) = tmpij(ils:ile,jls:int(jle/2))
        tmpij(ils:ile,jls:jle) = aicel(ils:ile,jls:jle,2)
        call getvara ('aicel', iou, ln, ib, ic, tmpij, c1, c0)
c        aicel(ils:ile,jls:jle,2) = tmpij(ils:ile,jls:jle)
        aicel(ils:ile,jls:int(jle/2),2) = tmpij(ils:ile,jls:int(jle/2))
      endif
