! source file: /data2/aschmitt/models/OESM/0.1/oesm_11_t020_short/uvic/updates/rivmodel.F
      subroutine rivmodel
...
!=======================================================================
      ...
      real icemsk(imt,jmt)
!     ice mask = 1 (no ice) = 0 (ice)
!     only in southern hemisphere to exclude precip over
!     Antarctica
      icemsk(:,:) = 1.0
      do j=2,int(jmt/2)
        do i=2,imtm1
          icemsk(i,j) = (1.-aicel(i,j,2))
        enddo
      enddo
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      do j=2,jmtm1
        do i=2,imtm1
          n = nriv(i,j)
          if (n .ne. 0) then
c            psum(n) = sbc(i,j,iro)*dxt(i)*cst(j)*dyt(j) + psum(n)
            psum(n) = sbc(i,j,iro)*dxt(i)*cst(j)*dyt(j)
c           remove runoff over interactive ice sheet
     &                *icemsk(i,j)
     &              + psum(n)
            psum_unmskd(n) = sbc(i,j,iro)*dxt(i)*cst(j)*dyt(j)
     &              + psum_unmskd(n)
          endif
        enddo
      enddo
      ...
      do j=2,jmtm1
        do i=2,imtm1
          ...
          if (n .ne. 0) then
            ...
            disch_unmskd(i,j) = psum_unmskd(n)*wdar(i,j)
!           add freshwater fluxes from interactive ice sheet
            disch(i,j) = disch(i,j) + runliq(i,j)
            disch_unmskd(i,j) = disch_unmskd(i,j) + runliq(i,j)
            disch_runliq(i,j) = runliq(i,j)
            sbc(i,j,isflx) = sbc(i,j,isflx) - socn*disch(i,j)
          endif
        enddo
      enddo
...
!     read the netcdf river file.
     ...
     &, c1, c0)
        nriv(2:imtm1,2:jmtm1) = tmpij(1:imtm2,1:jmtm2)
        call embmbc (nriv)
        call getvara ('L_rivdis', iou, imtm2*jmtm2, ib, ic, tmpij
     &, c1, c0)
        ndis(2:imtm1,2:jmtm1) = tmpij(1:imtm2,1:jmtm2)
        call embmbc (ndis)
...
! ericmod oct19 adding read of riv
...
! end ericmod oct19
! ericmod
        ! LDW: read SI runoff data
        print*, "fname = ",trim(fname)
        print*, "Just before runliq get var"
        call getvara ('runliq', iou, imtm2*jmtm2, ib, ic, tmpij, c1, c0)
        print*, "Just after runliq get var"
        runliq(2:imtm1,2:jmtm1) = tmpij(1:imtm2,1:jmtm2)
        call embmbc (runliq)
! ericnote, this is the orig vs i had for runfrz, not sure why
!        call getvara ('runfrz', iou, imt*jmt, ib, ic, runfrz, c1, c0)
        call getvara ('runfrz', iou, imtm2*jmtm2, ib, ic, tmpij, c1, c0)
        runfrz(2:imtm1,2:jmtm1) = tmpij(1:imtm2,1:jmtm2)
        call embmbc (runfrz)
      endif
! endericmo
