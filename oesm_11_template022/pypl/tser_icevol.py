...
f.close()
fl_time=array(time)
fl_toti=array(toti)/1.e6
print('time = ',fl_time)
print('ice vol = ', fl_toti)

figure(figsize=(9,6)) # w,h
#plot(fl_time3,fl_toti3,'r',label='control')
plot(fl_time,fl_toti,'r',label='test5')
#plot(fl_time,fl_toti,'k',label='no $\Delta$ice elev.')
#plot(fl_time2,fl_toti2,'k',label='test6')

legend(loc=2)

title('Total Ice Volume', fontsize=25)
xlabel('year', fontsize=20)
ylabel('10$^6$ km$^3$', fontsize=20)
#ylim([26.294, 26.296])
xlim([3500, 3600])
grid()
savefig('figs/ice_vol_ais_test.png')
show()
